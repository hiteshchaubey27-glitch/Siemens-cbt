...
function showInstructions() {
    ...
    const student = students.find(s => {
        const firstNameInDB = (s.name || "").split(' ')[0].trim().toLowerCase();
        const firstEnteredName = (enteredNameRaw || "").split(' ')[0].trim().toLowerCase();
        const dbPass = (s.pass || "").trim();
        const userPass = (enteredPass || "").trim();
        return firstNameInDB === firstEnteredName && dbPass === userPass;
    });
    ...
    if (student) {
        ...
        // Prepare 20 questions for this session
        currentQuestions = shuffle([...questionBank]).slice(0, 20);
        console.log("Login Successful: Session Loaded");
    } else {
...
function loadQuestion() {
    if (currentQuestions.length === 0) return; // Prevent loading if empty
